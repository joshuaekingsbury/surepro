...
for dye in ${dyes[*]}
do
	./obsExtractImg.sh ${obs} ${dye} ${dyeDir} ${scriptPath} ${obsPath}/${obs}/${evtPath} ${GTI_file} ${preRegImg/.NUM./$dye}
	wait $!
done
...
for dye in ${dyes[*]}
do
	./obsExtractAll.sh ${obs} ${dye} ${dyeDir} ${scriptPath} ${obsPath}/${obs}/${evtPath} ${GTI_file} ${histFile/DYE/$dye} ${obsPath}/${obs}/${obsDS9/.EXT./reg}
	wait $!
done
